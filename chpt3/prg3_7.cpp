...
//Declarations of scalars and vectors:
  ...
  cvec transmitted_symbols, received_symbols, afterChannel,receiveAfterDetection;           //cvec is a vector containing double_complex
  ...
  fad1.set_LOS_power(2.0); 
  cvec tc,abstc;
  tc.set_length(100000/2);
  abstc.set_length(100000/2);
  ...
  EbN0dB = linspace(0.0, 9.0, 20); //Simulate for 10 Eb/N0 values from 0 to 9 dB.
  cout<<EbN0dB<<endl;
  ...
  for (i = 0; i < EbN0dB.length(); i++) {
	...
	fad1.generate(transmitted_symbols.length(),tc);

   afterChannel = elem_mult(transmitted_symbols,tc);
  ...
  cout <<"*******"<< endl;
  cout<<bit_error_rate(0)<<endl;
  ofstream ff1("jieguo.txt");
  ofstream ff2("ebn0.txt");
  for(int m=0;m<EbN0dB.length();m++)
  {
    ff1<<bit_error_rate(m)<<endl;
	ff2<<EbN0dB(m)<<endl;
  }
    ff1.close();
    ff2.close();
