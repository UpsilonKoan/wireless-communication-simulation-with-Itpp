  ...
  const double N0 = 0.1; //噪声功率
{
    ...
    QAM qam(16);  //16-QAM
    int bps = round_i(qam.bits_per_symbol()); //每符号的比特数
    ...
    cvec tx_symbols = qam.modulate(tx_sym_numbers);  //符号调制
    cvec rx_symbols = tx_symbols + noise;   //接收符号
    ivec dec_sym_numbers = qam.demodulate(rx_symbols);  //符号解调
    ...
    tx_symbols = qam.modulate_bits(tx_bits);//比特调制
    rx_symbols = tx_symbols + noise; //接收符号
    bvec decbits = qam.demodulate_bits(rx_symbols);//比特硬解调
    vec softbits_approx = qam.demodulate_soft_bits(rx_symbols, N0, APPROX); //AWGN信道的软比特近似解调
